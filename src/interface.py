...
# --- INTEGRAÇÃO COM O CÉREBRO (ATUALIZADO FASE 3) ---
try:
    caminho_atual = os.path.dirname(os.path.abspath(__file__))
    if caminho_atual not in sys.path:
        sys.path.append(caminho_atual)
    
    # Agora importamos o CÉREBRO, que gerencia IA, Memória e Controle PC
    from cerebro import SiriusCerebro
    sirius_brain = SiriusCerebro() 
except ImportError as e:
    print(f"\033[31m[Erro]: Falha ao carregar o cérebro central: {e}\033[0m")
    sirius_brain = None
...
class SiriusWorker(QThread):
    ...
    def run(self):
        ...
        while self.rodando:
            ...
            # Prioridade 1: Comando de Texto (Manual)
            with self.lock:
                if self.comando_manual:
                    fala_usuario = self.comando_manual
                    self.comando_manual = None
            
            # Prioridade 2: Voz (apenas se não houve texto e modo voz ativo)
            if not fala_usuario and self.modo_voz_ativo:
                # DICA: Se o escutar_fluxo_continuo() travar aqui, 
                # o texto manual não vai entrar até ele liberar.
                try:
            ...
            if fala_usuario:
                ...
                if comando_str:
                    ...
                    # Chamada ao cérebro
                    if sirius_brain:
                        resposta_texto = sirius_brain.processar(comando_str)
                    else:
                        resposta_texto = "Conexão neural offline."

                    if resposta_texto:
                        self.resposta_pronta.emit(str(resposta_texto))
                        self.status_fala.emit(True) # Faz o Nexus 3D pulsar
                        try:
                            self.audio.falar(resposta_texto)
                        finally:
                            self.status_fala.emit(False)

            time.sleep(0.01) # Reduzi para 0.01 para maior precisão
# --- VISUALIZAÇÃO 3D (Mantida Sem Alterações) ---
class SiriusNexus3DView(QOpenGLWidget):
    def __init__(self, parent=None):
        ...
        self.pontos_nucleo = []
        for _ in range(1200):
            phi = random.uniform(0, 2 * math.pi); costheta = random.uniform(-1, 1)
            theta = math.acos(costheta); r = 2.4 
        ...
        for _ in range(500):
            r_core = random.uniform(0, 0.6); phi = random.uniform(0, 2 * math.pi); costheta = random.uniform(-1, 1)
            theta = math.acos(costheta); cx = r_core * math.sin(theta) * math.cos(phi); cy = r_core * math.sin(theta) * math.sin(phi); cz = r_core * math.cos(theta)
            self.pontos_nucleo.append([cx, cy, cz])

    @property
    def esta_falando(self): return self._esta_falando
    @esta_falando.setter
    def esta_falando(self, valor): self._esta_falando = valor; self.update()

    def update_animation(self):
        vel_rotacao = 1.2 if self._esta_falando else 0.4
        self.rotation += vel_rotacao; self.pulse += 0.08
        amplitude = 0.15 if self._esta_falando else 0.03
        factor = (1.0 + math.sin(self.pulse) * amplitude)
        for p in self.pontos_plexus: p['pos'] = [v * factor for v in p['orig']]
        self.update()
    ...
    def paintGL(self):
        ...
        glPushMatrix(); glRotatef(self.rotation, 0, 1, 0); glRotatef(self.rotation * 0.5, 1, 0, 0)
        ...
        glEnd(); glPopMatrix()
        glBegin(GL_POINTS)
        for p_core in self.pontos_nucleo: glColor4f(*cor_atual, random.uniform(0.4, 0.9)); glVertex3f(*p_core)
        glEnd()
...
class SiriusInterfaceMainWindow(QMainWindow):
    def __init__(self):
        ...
        self.btn_toggle.setStyleSheet("QPushButton { background: rgba(0,0,0,180); color: #5DE2FF; border: 1px solid #5DE2FF; border-radius: 8px; font-weight: bold; } QPushButton:hover { background: rgba(93, 226, 255, 0.2); }")
        self.btn_toggle.clicked.connect(self.alternar_interface)
        
        self.hud_chat = QFrame()
        self.hud_chat.setFixedHeight(300)
        self.hud_chat.setStyleSheet("QFrame { background-color: rgba(5, 5, 5, 240); border-top: 2px solid #5DE2FF; border-top-left-radius: 30px; border-top-right-radius: 30px; }")
        ...
        self.input_texto.setStyleSheet("QLineEdit { background: rgba(93, 226, 255, 0.05); border: 1px solid #5DE2FF; border-radius: 15px; color: white; padding: 12px; }")
        ...
        # Inicializa o Worker passando o sistema de áudio
    ...
    def alternar_interface(self):
        ...
        if not nova_visibilidade: 
            with self.worker.lock: self.worker.comando_manual = None
        self.btn_toggle.setText("VOZ" if nova_visibilidade else "TEXTO")
        if nova_visibilidade: self.input_texto.setFocus()
