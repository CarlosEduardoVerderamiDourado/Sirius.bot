...
class SiriusNeuronio:
    ...
    def arquivar_conhecimento(self, df):
        try:
            ...
            df_apenas_novos = pd.read_sql_query("SELECT conteudo, tema FROM conhecimento_geral", conn2)
            conn2.close()
            
            df_apenas_novos.to_sql("memoria_permanente", conn, if_exists="append", index=False)
