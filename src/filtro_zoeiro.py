...
class SiriusFiltro:
    ...
    def aplicar_zoeira(self, texto):
        # Respostas curtas, com erro ou código saem sem filtro
        if "```" in texto or "Erro" in texto or len(texto) < 20:
            return texto

        # Remove formalidades que a IA às vezes coloca sem querer
        texto = texto.replace("Claro!", "").replace("Com certeza!", "").strip()

        # Sorteio de estilo (pra não ficar repetitivo)
